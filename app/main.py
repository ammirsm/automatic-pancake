...
from configs import model_configs, strategies, number_of_iterations, cycle, title_data
...
for strategy in strategies:
    ...
    for i in range(number_of_iterations):
        ...
        for key, config in model_configs.items():
            config["number_of_relevant"] = config["data"].data.loc[config['data'].data.label == 1].shape[0]
            ...
            model_configs[key]["learning_model"] = LearningModel(config["data"],
                                                                 feature_columns=config["feature_columns"],
                                                                 model=config["model"],
                                                                 the_percentile=config["percentile"])
            model_configs[key]["agent"] = ActiveLearningAgent(learning_model=config["learning_model"], name=key,
                                                              each_cycle=cycle,
    ...
    strategies_result[str(strategy[0]) + " - " + str(strategy[1])] = list_of_models_config
