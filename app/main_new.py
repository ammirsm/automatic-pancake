...
from configs import cycle, model_configs, number_of_iterations, query_strategies, result
...
def start_active_learning(key, config, strategies_result, strategy_key, dirs):
    config["data"].data["label"] = config["data"].data[config["label"]]
...
if __name__ == "__main__":
    ...
    for query_strategy in query_strategies:
        strategy_name = str(query_strategy[0]) + " - " + str(query_strategy[1])
        strategies_result[strategy_name] = []
        for i in range(number_of_iterations):
            for key, config in model_configs.items():
                ...
                path = start_active_learning(
                    ...
                    strategies_result,
                    strategy_name,
                    [now, strategy_name, str(i)],
                )
                counter += 1

    second = timeit.default_timer()
    draw_helper(os.path.join(result, now))
